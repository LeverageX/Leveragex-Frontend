...
const PnL = () => {
  ...
  const fetchUserStocks = async () => {
    try {
      const response = await axios.get(
        `https://leveragex-backend.onrender.com/api/users/stocks/${userId}`
      );
  ...
  const fetchRealTimePrices = async () => {
    try {
      ...
      const response = await axios.get(
        `https://leveragex-backend.onrender.com/api/watchlist${watchlistType}`
      );
  ...
  const handleSell = async (stockName, quantity, autoSell = false) => {
    try {
      ...
      const response = await axios.post(
        "https://leveragex-backend.onrender.com/api/users/sell",
        {
          userId,
          stockName,
          quantity,
