...
function Plans() {
    ...
    useEffect(() => {
        ...
        const fetchUserPlanStatus = async () => {
            try {
                const response = await axios.get(`https://leveragex-backend.onrender.com/api/plans/user-plan/${userId}`);
                if (response.data) {
    ...
    const handlePayment = async () => {
        try {
            const userId = localStorage.getItem('userId');
            const response = await axios.post('https://leveragex-backend.onrender.com/api/plans/purchase', { userId, plan: selectedPlan });
