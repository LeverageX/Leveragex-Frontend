...
export async function savePnLDataToBackend(pnlData) {
  try {
    const response = await axios.post('https://leveragex-backend.onrender.com/api/pnl/save', {
...
export async function loadPnLDataFromBackend(userId) {
  try {
    const response = await axios.get(`https://leveragex-backend.onrender.com/api/pnl/${userId}`);
